...
I main(){
X=T(q,64);g();I f,z,s,id,fr=400,x,y,i,tx,ty;F t,u,v,d,k,tr,bt,at,r,g,b,rad,bright;
J<V>CT;V ti,ld,n,m,p,ro=w(0,0,-1),rd;
char fn[16];FILE*o;
O(f=0;f<fr;f++){
snprintf(fn,16,"%03d.ppm",f);
if(!(o=fopen(fn,"wb")))continue;
fprintf(o,"P6\n%d %d\n255\n",W,W);
...
r=g=b=0;
if(id){
p=A(ro,M(rd,d));n=C(p,CT,ti,tr);
bright=.3+fmax(0.f,n.x*ld.x+n.y*ld.y+n.z*ld.z)*.7;
bt=(z>=0)?(F)z/(CT.size()-1):0;
at=fmod((id==1?sqrt(bt):1)+f*y3,1);m=U(at);
r=bright*m.x;g=bright*m.y;b=bright*m.z;
